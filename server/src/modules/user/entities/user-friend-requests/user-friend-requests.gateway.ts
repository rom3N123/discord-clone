...
export class UserFriendRequestGateway {
  ...
  async create(
    ...
    @MessageBody() { requestTo }: Pick<CreateUserFriendRequestDto, 'requestTo'>,
    ...
    try {
      ...
      const socketIds = usersOnline.getSocketsByUsersIds([requestTo, _id]);
  ...
  async cancel(
    ...
    try {
      const {
        userFriendRequest: { _id: friendRequestId, requestBy, requestTo },
  ...
  async accept(
    ...
    try {
      ...
      usersOnline.ifOnline(newFriend._id, (socketId) => {
        this.wss
          .to(socketId)
          .emit(`friends~accepted`, { requestId, newFriend: updatedUser });
      });
    } catch (error) {
